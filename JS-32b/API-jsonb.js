let url = "https://std.stegion.nl/cs_codebase/js31_api_base/users.json";
...
function getHtmlFromUrl(url) {  // haal dat van URL binnen, verwerkt dit in een HTML-table en plaatst {
    fetch(url)
        ...
        .then(json => {
            ...
            document.getElementById("output1").innerHTML = htmlUsersTable;
        });
}
...
async function fGetData(url) {
    ...
    localStorage.setItem(url, JSON.stringify(json)); // Caching
    return json;
}
